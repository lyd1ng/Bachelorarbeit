...
fp clamp(fp x, fp upper, fp lower)
{
    return fmin(upper, fmax(x, lower));
}

fp smoothstep(fp edge0, fp edge1, fp x)
{
    fp t;
    t = clamp((x - edge0) / (edge1 - edge0), 1.0, 0.0);
...
void update_fields()
{
	...
	current_ez_universe[middle_x][middle_y][middle_z] = smoothstep(0, ramped_sin_length * time_step, current_time) * sin(2.0 * M_PI * ramped_sin_frequency * current_time);
