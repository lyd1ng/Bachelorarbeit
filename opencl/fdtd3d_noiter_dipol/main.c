...
int main(int argc, char** argv)
{
	...
	while (current_time < max_time)
	{
		...
		old_e_universe[sid].x = ramped_sinus(current_time, ramped_sin_length * time_step, ramped_sin_amplitude, ramped_sin_frequency) / grid_width_x;
	...
	errNum = clEnqueueReadBuffer(queue, current_h_universeb, CL_TRUE, 0, USIZE * USIZE * USIZE * sizeof(fp4),
		(void*)(current_h_universe), 0, NULL, NULL);
	
	// Print the results in an ugly list
	for (int z = USIZE / 2 + 1; z < USIZE / 2 + 2; z++)
	{
		for (int y = 0; y < USIZE; y++)
		{
			for (int x = 0; x < USIZE; x++)
			{
				...
				fprintf(stderr, "%d\t%d\t%d\t%.32f\t%.32f\t%.32f\n", x, y, z, current_h_universe[id].x, current_h_universe[id].y, current_h_universe[id].z);
				// printf("%d\t%d\t%d\t%.32f\t%.32f\t%.32f\n", x, y, z, sigmax[id], sigmay[id], sigmaz[id]);
				//printf("%d\t%d\t%.32f\n", x, y, sqrt(pow(current_e_universe[id].x, 2) + pow(current_e_universe[id].y, 2) + pow(current_e_universe[id].z, 2)));
			}
