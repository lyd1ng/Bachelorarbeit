...
sigma_universe = 0 * x_indices
e_factor1_universe = 0 * x_indices
e_factor2_universe = 0 * x_indices
h_factor1_universe = 0 * x_indices
h_factor2_universe = 0 * x_indices
...
def set_pml_geometry():
    """
    Set the pml material using sigma terms. Only cells within the pml
    material has to be set as the sigma universe is initialised with
    zeroes.
    """
    pml_width = 25
    f = vacuum_permittivity / (2.0 * time_step)
    for x in range(0, pml_width):
        sigma_universe[pml_width - x] = f * (x / pml_width)**3
        sigma_universe[len(x_indices) - pml_width + x] = f * (x / pml_width) ** 3
...
def calculate_factors():
    ...
    for x in range(0, len(x_indices)):
        e_factor1_universe[x] = 1 - ((time_step * sigma_universe[x]) / vacuum_permittivity)
        e_factor2_universe[x] = get_electrical_update_coefficient_1d(time_step,
                               permittivity_universe[x])
        h_factor1_universe[x] = (-sigma_universe[x] / (2 * vacuum_permittivity) + 1.0 / time_step) / (sigma_universe[x] / (2 * vacuum_permittivity) + 1.0 / time_step)
        h_factor2_universe[x] = (light_speed / permeability_universe[x]) / (sigma_universe[x] / (2 * vacuum_permittivity) + 1.0 / time_step)
...
def update_fields():
    ...
    current_magnetical_universe[-1] = h_factor1_universe[-1] * old_magnetical_universe[-1] + h_factor2_universe[-1] * ((last_last_upper_boundary_value - old_electrical_universe[-1]) / grid_width)

    # Calculate the magnetic field from the old electrical field
    for x in range(0, len(x_indices) - 1):
        current_magnetical_universe[x] = h_factor1_universe[x] * old_magnetical_universe[x] + h_factor2_universe[x] * ((old_electrical_universe[x + 1] - old_electrical_universe[x]) / grid_width)
    ...
    # universe using the dirichlet boundary condition
    current_electrical_universe[0] = e_factor1_universe[0] * old_electrical_universe[0] + e_factor2_universe[0] * ((current_magnetical_universe[0] - last_last_lower_boundary_value) / grid_width)
    ...
    for x in range(1, len(x_indices)):
        current_electrical_universe[x] = e_factor1_universe[x] * old_electrical_universe[x] + e_factor2_universe[x] * ((current_magnetical_universe[x] - current_magnetical_universe[x - 1]) / grid_width)
