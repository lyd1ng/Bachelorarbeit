...
x_indices = np.linspace(0, 20, 21, dtype=np.float64)
y_indices = np.linspace(0, 500, 501, dtype=np.float64)
...
def set_geometry():
    ...
    for y in range(0, len(y_indices)):
        permittivity_universe[1][y] = math.inf
    for x in range(0, len(x_indices)):
        permittivity_universe[x][1] = math.inf
    pass
...
def update_image(*args):
    ...
    """
    update_fields()
    image.set_data(current_ez_universe * current_hx_universe * current_hy_universe)
    return image,
...
if __name__ == "__main__":
    ...
    anim = animation.FuncAnimation(figure, update_image, save_count=1000,
                                        interval=5, blit=True)
    anim.save('waveguide_correct.mp4', writer=writer)
    # plt.show()
